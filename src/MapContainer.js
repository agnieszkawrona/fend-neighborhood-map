...
class MapContainer extends Component {
  ...
  fetchFsVenData = (latlng) => {



    fetch(`https://api.foursquare.com/v2/venues/search?ll=${latlng}&limit=1&client_id=SU0CUUABMIBUIYGLXV30NT1KITT51TWKKLPXRYKG3PZGUAQX&client_secret=4USL5J5MTCAQLC2MGHNE0TGEZB32LFG0ZJYUQRUNWVVQMGXP&v=20180822`).then( response => {
         return response.json()
      }).then(venueName => {
        this.setState({
          venueName: response.venues.name,
        })
        console.log(this.state.venueName)
      });
  }
  ...
  componentDidMount() {
    ...
    this.fetchFsVenData("51.10470549999999,17.0313717")
  }
  ...
  initMap = () => {
    const { locations, markers, map } = this.state
    ...
    this.state.locations.forEach((location, i) => {
      ...
      let stringCoord = (String(this.state.locations[i].locForFs))
      //infoWindow for a location container: info from foursquare API
      this.fetchFsVenData(stringCoord)
      console.log(this.state.venueName)
      
      var locationInfo = 
        '<div class="info-window">'+
          '<h1 class="info-window-heading">'+marker.title+'</h1>'+
          '<div class="info-window-content">'+
            '<p>'+this.state.venueName+'</p>'+
            '<p>'+this.state.venueAddress+'</p>'+
          '</div>'+
        '</div>';
  ...
  filterLocations = (query) => {
    ...
    if (query) {
      ...
      this.setState({filteredPlaces: locations.filter((loc) => match.test(loc.name)),
